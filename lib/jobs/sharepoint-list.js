...
const { storageAccount, graphUrl } = require('../../config')
const { logger } = require('@vtfk/logger')
const { get } = require('@vtfk/azure-blob-client')
const graphToken = require('../get-graph-token')
const sharepointToken = require('../get-sharepoint-token')
...
module.exports = async (jobDef, flowStatus) => {
  ...
  if (mapper) {
    ...
    listItems = mapper(flowStatus)
  } else {
    logger('info', ['sharePointList', 'No mapper defined in options', 'using default'])
    throw new Error('No default mapper is set up for SharePoint list. Please provide a custom mapper in flow definition')
  }
  ...
  const fs = require('fs')
  fs.writeFileSync('./batchrequest.json', JSON.stringify(payload, null, 2))
  ...
  const { retryPayload, uploadFileItems } = repackBatchRequests(data, payload, listItems, flowStatus.parseXml.result.files)
  fileUploads = [...fileUploads, ...uploadFileItems]
  const failedFileUploads = []

  for(const fileUpload of fileUploads) {
    const sharepointToken = await getSharepointToken()
   
    // hente fil fra blob som b64, lage buffer av fila og dytte videre til Rest-api. Husk Try-catch
  }
  // gå gjennom fileUploads lista. For hvert element; laste opp fil og/eller vedlegg på listeelementets interne sharepoint id. Hvis det ikke går bra, legg den til i faileduplods liste. 
  // hvis ja, last opp det som skal lastes opp og legg dette ved listeelementet
  // lage en retryPayload for filene i tillegg

  if (retryPayload.requests.length > 0 || failedFileUploads.length > 0) {
    flowStatus.sharepointList.graphResult = data
...
// Lage feilhåndtering: det første som skjer er å sjekke om vi har et resultat allerede som er lagret i blob. I så fall, la en ny batchrequest av de som feilet, og kjør på nytt. Hvis ikke, kjør hele requesten.
// se på vedleggsdritten 
