...
const { storageAccount, willNotRunAgainFilename, roomServiceTeamsWebhook } = require('../../config')
...
module.exports = async () => {
  ...
  const teamsMsg = {
    ...
    sections: [
      ...
      {
        activityTitle: `**${willNotRunAgainBlobs.length}** blob(er) har har blitt forsøkt maksimalt antall ganger og vil ikke bli kjørt igjen`,
        activitySubtitle: 'Dette er blober som har kjørt (og feilet) sitt maksimale antall ganger. Se etter feil i flow-status.json fila, rett feilene, slett will-not-run-again.json-fila og rediger (eller slett om alt skal kjøres på nytt) flow-status.json og vent på neste kjøring',
      ...
      },
      {
        ...
        activitySubtitle: 'Dette er skjemaer som enten venter i kø eller er satt til å ikke bli slettet (doNotDeleteBlobs = true i flowDef-fila)',
        facts: [],
        markdown: true
      }
    ]
  }
  await axios.post(roomServiceTeamsWebhook, teamsMsg)
}
