...
const { storageAccount, graphUrl, sharepointClient } = require('../../config')
...
module.exports = async (jobDef, flowStatus) => {
  ...
  for(const fileUpload of fileUploads) {
    try {
      logger('info', ['Starting file upload to SharePoint', 'fileName', fileUpload.file.name, 'fileDesc', fileUpload.file.desc, 'siteName', fileUpload.siteName])
      ...
      logger('info', ['Successfully uploaded file to SharePoint', 'fileName', fileUpload.file.name, 'fileDesc', fileUpload.file.desc, 'siteName', fileUpload.siteName])
    } catch (error) {
      failedFileUploads.push(fileUpload)
      const errorMsg = error.response?.data || error.stack || error.toString()
      logger('warn', ['Failed when uploading file to SharePoint', 'fileName', fileUpload.file.name, 'fileDesc', fileUpload.file.desc, 'siteName', fileUpload.siteName, 'error', errorMsg])
    }
  }
