...
const { archive, storageAccount, autoConvertFileFormats } = require('../../config')
...
module.exports = async (jobDef, flowStatus) => {
  ...
  if (mapper) {
    ...
    const { Fnr, Fornavn, Etternavn, AnsVirksomhet, Tilgangsgruppe, Tittel } = xmlData
  ...
  logger('info', ['archive', 'Successfully archived document'])
  return data
}
